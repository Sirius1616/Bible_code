...
def check_message(x_coord, acceptable_coords, variance):
    """Return YES if within acceptable ± variance, MISALIGNED if outside, else COULD NOT FIND"""
    if not x_coord:
...
def match_subheads_with_spans(cleaned_file, span_file, output_file, standards_file):
    """Match cleaned subheads with span contents from Excel, then check standards.
       All comments go in Column G (Message). COULD NOT FIND or MISALIGNED rows go first."""
    ...
    with open(cleaned_file, "r", newline="", encoding="utf-8") as infile:
        ...
        for row in reader:
            ...
            if match:
                ...
                message = check_message(x_coord, acceptable_coords, variance)
    ...
    # Reorder: COULD NOT FIND or MISALIGNED first
    errors_first = [
        r for r in results if r["Match Status"] == "COULD NOT FIND" or r["Message"] == "MISALIGNED"
    ]
    matches_after = [
        r for r in results if not (r["Match Status"] == "COULD NOT FIND" or r["Message"] == "MISALIGNED")
    ]
